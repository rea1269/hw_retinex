...
class LoRALinear(nn.Module):
    def __init__(self, in_features, out_features, r=16, lora_alpha=8):
        ...
        self.lora_A.is_lora = True
        self.lora_B.is_lora = True
...
class IG_MSA(nn.Module):
    def __init__(
        ...
        self.to_q_lora = LoRALinear(dim, dim_head * heads, r=16, lora_alpha=8)
        self.to_k_lora = LoRALinear(dim, dim_head * heads, r=16, lora_alpha=8)
        self.to_v_lora = LoRALinear(dim, dim_head * heads, r=16, lora_alpha=8)
        self.rescale = nn.Parameter(torch.ones(heads, 1, 1))
        self.proj = nn.Linear(dim_head * heads, dim, bias=True)
        self.proj_lora = LoRALinear(dim_head * heads, dim, r=16, lora_alpha=8)
    ...
    def forward(self, x_in, illu_fea_trans):
        ...
        # lora_out = self.to_q_lora(x)
        # base_out = self.to_q(x)
        # # 打印两者的范数比例
        # print(f"Ratio: {lora_out.norm() / base_out.norm():.4f}")
        q_inp = self.to_q(x) + self.to_q_lora(x)
        k_inp = self.to_k(x) + self.to_k_lora(x)
        v_inp = self.to_v(x) + self.to_v_lora(x)
        ...
        proj_out = self.proj(x) + self.proj_lora(x)
        out_c = proj_out.view(b, h, w, c)
...
class FeedForward(nn.Module):
    def __init__(self, dim, mult=4):
        ...
        # 在外部挂载两个纯粹的 LoRA 旁路
        self.lora_0 = LoRAConv1x1_Only(dim, dim * mult, r=16, lora_alpha=8)
        self.lora_4 = LoRAConv1x1_Only(dim * mult, dim, r=16, lora_alpha=8)

    def forward(self, x):
        # out = self.net(x.permute(0, 3, 1, 2).contiguous())
        x_in = x.permute(0, 3, 1, 2).contiguous()
        
        # 手动分解 Sequential注入 LoRA
        x1 = self.net[0](x_in) + self.lora_0(x_in)  # 第一层 1x1 卷积 + 旁路
        x2 = self.net[1](x1)                        # GELU
        x3 = self.net[2](x2)                        # 中间的 3x3 DW 卷积（不动）
        x4 = self.net[3](x3)                        # GELU
        out = self.net[4](x4) + self.lora_4(x4)     # 最后一层 1x1 卷积 + 旁路
...
class Denoiser(nn.Module):
    ...
    def _init_weights(self, m):
        ...
        if getattr(m, 'is_lora', False):
            ...
            return
        if isinstance(m, LoRALinear) or isinstance(m, LoRAConv1x1_Only):
            #print("not for lora layers!!!!!")
            return
